...
| 模块 | 说明 | 默认 |
| --- | --- | --- |
| 色情 / 擦边内容 | 112 个强词 + 46 个弱词（中英双语），强命中直接折叠，弱词累积后模糊 | 开 |
| 垃圾广告 / 引流诈骗 | 93 个强词 + 32 个弱词，覆盖加微信、刷单、博彩、贷款、代开发票等；含微信号 / QQ / 手机号 / 群号 / 可疑域名等正则 | 开 |
| 无效灌水 / 重复刷屏 | 33 个灌水词（沙发、打卡、前排…）+ 同页重复文本检测（同一段文本出现 ≥4 次自动折叠） | 开 |
...
```bash
npm test      # 20 项引擎单元测试（关键词、阈值、误伤、域名匹配、肤色算法）
...
广告文案/色情文案/贷款广告被折叠、正常段落不被误伤、重复内容被清理、肤色图片被模糊、
正常图片不误判、角标出现、后台 Service Worker 正确加载引擎，并额外检查弹窗页与设置页无脚本报错、
规则测试器判定正确。
