...
| 模块 | 说明 | 默认 |
| --- | --- | --- |
| 色情 / 擦边内容 | 112 个强词 + 57 个弱词（中英双语），强命中直接折叠，弱词累积后模糊 | 开 |
| 垃圾广告 / 引流诈骗 | 96 个强词 + 36 个弱词，覆盖加微信、刷单、博彩、贷款、代开发票等；含微信号 / QQ / 手机号 / 群号 / 可疑域名等正则 | 开 |
| 无效灌水 / 重复刷屏 | 33 个灌水词（沙发、打卡、前排…）+ **僵尸号群发检测**：同组文本出现 ≥3 次时，会把该组**已出现的条目一并回溯折叠** | 开 |
...
```bash
npm test      # 23 项引擎单元测试（关键词、阈值、误伤、域名匹配、肤色算法、刷屏指纹）
