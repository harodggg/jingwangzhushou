...
| 垃圾广告 / 引流诈骗 | 94 个强词 + 31 个弱词 + 12 个低置信信号，覆盖加微信、刷单、博彩、贷款、代开发票等；含微信号 / QQ / 手机号 / 群号 / 可疑域名等正则 | 开 |
| 无效灌水 / 重复刷屏 | 33 个灌水词（沙发、打卡、前排…）+ 僵尸号群发检测（默认保守：同页相同内容 ≥4 条才折叠超出的；可切「严格」为 ≥3 条整组回溯） | 开 |
| 图片裸露识别 | 本地像素分析：肤色占比 + 皮肤平滑度 + 画面彩色度 + **人脸特征排除** + 纯色图排除，命中即模糊，点击可查看 | 开 |
...
- **右下角角标** 显示本页已过滤数量：点「显示」一键恢复全部内容，点「误报」打开本页过滤清单
- **误报纠正**：清单里每一条都能点「误报，不再过滤」——文本会加入本地白名单词，图片会加入图片白名单，之后永不再拦
- **右键菜单**：重新扫描本页 / 显示被过滤内容 / 将本站加入白名单 / 加入拦截名单 / 打开设置
- **工具栏弹窗**：总开关、分类开关、过滤强度（1 宽松 ~ 5 严格）、当前站点白名单、实时统计、误报清单入口
...
- **标题（H1~H4）与长文本（≥120 字）只模糊、不折叠**：整段正文或标题消失的代价太大，
  而真正的垃圾广告几乎都是短句，仍然照旧折叠
- 任何被过滤/模糊的内容都可以一键恢复；点角标「误报」可把误判的内容加入白名单，**之后永不再拦**

图片判定（`src/common/filter-engine.js` 的 `skinScore`）：对缩略后的像素做肤色分类统计，
综合以下信号给出 0~1 的置信度，超过阈值才模糊：
...
| 皮肤平滑度 | 皮肤区域越平滑越像裸露 |
| 画面彩色度 | 越五彩斑斓越像正常照片 |
| **人脸特征密度** | 肤色区域内夹着明显暗部（眼睛/眉毛/嘴/头发）→ 判定为人像特写，得分 ×0.45 |
| **纯色占比** | 整图几乎是同一个颜色 → 占位图/纯色背景，直接排除 |

此外，页头/页脚/导航/侧边栏里的图片（站点素材）、边长 <110px 的小图、头像 logo 类图片
一律不分析。这是**启发式**方法，不是机器学习模型，仍可能有例外——
所以设计上只做「先模糊、可点开」，并且提供了逐图「误报」入口。
...
- 调整过滤方式、强度、图片肤色阈值、最短过滤文本长度、**重复内容检测策略**、角标位置
- 维护站点**白名单**与**自定义拦截域名**
- 追加自己的色情 / 垃圾关键词，以及**白名单词**
- 查看 / **清空误报记录**（白名单词与图片白名单）
- 用 **规则测试器** 粘贴任意文本，立即看到分数、命中的规则与最终判定
- **导出 / 导入** JSON 配置，便于备份或多机迁移

**被误伤时的三种纠正方式**：

1. 单个条目：点页面右下角角标 →「误报」→ 在清单里点「误报，不再过滤」（最精准，永久生效）
2. 某个词：设置页把该词加进「白名单词」
3. 整个网站：弹窗点「加入白名单」，或右键 → 将本站加入白名单
...
```bash
npm test      # 28 项引擎单元测试（关键词、阈值、误伤、域名匹配、肤色算法、刷屏指纹）
...
广告文案/色情文案/贷款广告被折叠、正常段落与标题不被误伤、表情变体刷屏在保守模式下只折叠超出的
（切到严格模式则整组回溯）、肤色照片被模糊、**人像特写与纯色占位图不被误判**、
角标与误报入口出现、后台 Service Worker 正确加载引擎、弹窗页与设置页无脚本报错、
**「误报」流程生效且刷新后仍然有效**。
